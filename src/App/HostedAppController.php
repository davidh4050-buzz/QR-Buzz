...
class HostedAppController {
    ...
    public function __construct() {
        ...
        $this->payloads = new QRPayloadService($this->types);
    ...
    public function route(): void {
        ...
        if ($path === 'pricing') { $this->page('Pricing', $this->pricing(), false); }
        if ($path === 'register') { if (is_user_logged_in()) { $this->redirectAfterLogin(); } $this->page('Create account', $this->registerForm(), false); }
        if ($path === 'login') { if (is_user_logged_in()) { $this->redirectAfterLogin(); } $this->page('Log in', $this->loginForm(), false); }
        if ($path === 'forgot-password') { $this->page('Password recovery', $this->passwordPage(), false); }
    ...
    private function postOnboarding(): void {
        ...
        if ($action === 'plan') {
            ...
            if ($plan === 'free') { $this->workspaceRepo->updatePlan($workspace->id, 'free'); $this->subscriptions->upsert($workspace->id, ['user_id' => get_current_user_id(), 'plan_key' => 'free', 'status' => 'free']); $this->workspaceRepo->updateOnboarding($workspace->id, 'pending', 'workspace'); $this->profiles->updateOnboarding(get_current_user_id(), 'pending', 'workspace'); $this->redirect('/app/onboarding?step=workspace'); }
            ...
            $this->redirect('/app/onboarding?step=plan&error=stripe');
        }
    ...
    private function postQr(): void {
        ...
        if (!$this->payloads->isPayloadValid($type, $staticPayload)) { $this->redirect('/app/qr/new?error=invalid'); }
    ...
    private function postAccount(): void { check_admin_referer('qrbuzz_app_account', 'nonce'); $userId = get_current_user_id(); wp_update_user(['ID' => $userId, 'first_name' => sanitize_text_field((string) ($_POST['first_name'] ?? '')), 'last_name' => sanitize_text_field((string) ($_POST['last_name'] ?? ''))]); if (!empty($_POST['password']) && $_POST['password'] === ($_POST['password_confirm'] ?? '')) { wp_set_password((string) $_POST['password'], $userId); wp_set_auth_cookie($userId); } $this->redirect('/app/settings/account?saved=1'); }
    ...
    private function appPage(string $path): void {
        $workspace = $this->workspaces->current();
        if (!$workspace->onboardingComplete() && $path !== 'app/onboarding') { $this->redirect('/app/onboarding'); }
        if ($path === 'app' || $path === 'app/dashboard') { $this->app('Dashboard', $this->dashboard()); }
        if ($path === 'app/onboarding') { $this->page('Onboarding', $this->onboarding(), true); }
    ...
    private function dashboard(): string { $s = $this->qrCodes->dashboardSummary(); $e = $this->entitlements->summary(); return $this->hero('Welcome to ' . esc_html($this->workspaces->current()->name), '<a class="qrb-button qrb-button-primary" href="' . esc_url(home_url('/app/qr/new')) . '">Create QR</a>') . $this->metrics([['QR assets',$s['total_qr_codes']],['Total scans',$s['total_scans']],['Campaigns',count($this->campaigns->active())],['Plan',$e['plan']['label']]]) . '<section class="qrb-card"><h2>Quick actions</h2><div class="qrb-actions"><a class="qrb-button" href="/app/qr/new?type=dynamic_url">Dynamic QR</a><a class="qrb-button" href="/app/qr/new?type=static_url">Static QR</a><a class="qrb-button" href="/app/qr/new?type=wifi">WiFi QR</a><a class="qrb-button" href="/app/campaigns">New Campaign</a></div></section>' . $this->usageCard(); }
    ...
    private function qrForm(): string { $type = $this->types->normalize(sanitize_key((string) ($_GET['type'] ?? 'dynamic_url'))); $campaigns = '<option value="0">Unassigned</option>'; foreach ($this->campaigns->active() as $c) { $campaigns .= '<option value="' . esc_attr($c->id) . '">' . esc_html($c->name) . '</option>'; } return '<h1>New QR</h1><form class="qrb-card qrb-form" method="post">' . wp_nonce_field('qrbuzz_app_qr', 'nonce', true, false) . '<label>Name<input name="name" required></label><label>Type<select name="type"><option value="dynamic_url" ' . selected($type, 'dynamic_url', false) . '>Dynamic URL</option><option value="static_url" ' . selected($type, 'static_url', false) . '>Static URL</option><option value="wifi" ' . selected($type, 'wifi', false) . '>WiFi</option><option value="vcard" ' . selected($type, 'vcard', false) . '>Business Card</option><option value="text" ' . selected($type, 'text', false) . '>Text</option></select></label><label>URL or primary content<input name="payload[destination_url]" type="url" placeholder="https://example.com"></label><label>WiFi SSID<input name="payload[ssid]"></label><label>WiFi password<input name="payload[password]"></label><label>Text<textarea name="payload[text]"></textarea></label><label>Campaign<select name="campaign_id">' . $campaigns . '</select></label><button class="qrb-button qrb-button-primary">Create QR</button></form>'; }
    private function qrDetail(int $id): string { $qr = $this->qrCodes->find($id); if (!$qr) { return '<h1>QR not found</h1>'; } return '<h1>' . esc_html($qr->name) . '</h1><div class="qrb-card"><p><strong>Type:</strong> ' . esc_html($this->types->label($qr->type)) . '</p><p><strong>Destination:</strong> ' . esc_html($qr->destinationUrl ?: $qr->staticPayload) . '</p><p><strong>Scans:</strong> ' . esc_html((string) $qr->scanCount) . '</p><p><code>' . esc_html($qr->shortcode) . '</code></p></div>'; }
    ...
    private function accountSettings(): string { $u = wp_get_current_user(); $p = $this->profiles->profile($u->ID); return '<h1>Account</h1><form class="qrb-card qrb-form" method="post">' . wp_nonce_field('qrbuzz_app_account', 'nonce', true, false) . '<label>First name<input name="first_name" value="' . esc_attr($u->first_name) . '"></label><label>Last name<input name="last_name" value="' . esc_attr($u->last_name) . '"></label><p>Email verification: ' . esc_html($p && $p->email_verified ? 'Verified' : 'Not verified') . '</p><label>New password<input type="password" name="password"></label><label>Confirm password<input type="password" name="password_confirm"></label><button class="qrb-button qrb-button-primary">Save account</button></form>'; }
    ...
    private function verifyEmail(): void { $userId = absint($_GET['user'] ?? 0); $token = (string) ($_GET['token'] ?? ''); $ok = $userId && $token && $this->profiles->verifyByToken($userId, $token); $this->page('Verify email', '<h1>' . ($ok ? 'Email verified' : 'Verification failed') . '</h1><p><a class="qrb-button" href="/app/dashboard">Continue</a></p>', false); }
    ...
    private function page(string $title, string $content, bool $app): void { status_header(200); nocache_headers(); echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . esc_html($title) . '</title>' . $this->styles() . '</head><body class="qrb-body"><main class="qrb-public"><a class="qrb-logo" href="/">QR Buzz</a>' . $content . '</main></body></html>'; exit; }

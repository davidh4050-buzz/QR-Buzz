...
class HostedAppController {
    ...
    private function studioDesignFields(QRDesignSettings $design): string {
        ...
        $html .= '</select></label>' . $this->input('Quiet zone / margin', 'margin', (string) $design->margin, 'number') . $this->logoField($design) . $this->input('Logo size (%)', 'logo_size', (string) $design->logoSize, 'number') . '<label><input type="checkbox" name="set_as_brand_kit" value="1"> Set as Brand Kit defaults</label>' . $logoNote . '</div></details>';
        if (!$this->entitlements->allows('advanced_branding')) {
            return $html . '<details class="qrb-studio-section" data-qrb-accordion="advanced-style"><summary>Advanced style</summary><div class="qrb-studio-section-body"><p class="qrb-alert">Advanced QR styling is available on Pro and Business plans.</p></div></details>';
        }

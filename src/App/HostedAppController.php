...
class HostedAppController {
    ...
    private function billingSettings(): string { $e = $this->entitlements->summary(); $sub = $e['subscription']; return $this->settingsNav('billing') . $this->pageHeader('Billing', 'Review plan status and usage limits.') . '<div class="qrb-card"><p><strong>Current plan:</strong> ' . esc_html($e['plan']['label']) . '</p><p><strong>Status:</strong> ' . esc_html($sub['status']) . '</p><p><strong>Renewal:</strong> ' . esc_html($sub['renewal_date'] ?: '-') . '</p><p class="qrb-alert">Stripe is in test-mode prototype configuration for v0.9.6. If test keys are not configured, paid plan selection runs in local prototype mode.</p><a class="qrb-button" href="/app/onboarding?step=plan">Change plan</a></div>' . $this->usageCard(); }

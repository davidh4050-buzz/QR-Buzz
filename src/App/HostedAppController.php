...
class HostedAppController {
    ...
    private function postQr(): void {
        ...
        if (!$this->payloads->isPayloadValid($type, $staticPayload)) { $this->redirect('/app/qr/new?type=' . rawurlencode($type) . '&error=invalid'); }
    ...
    private function qrForm(): string {
        $type = $this->types->normalize(sanitize_key((string) ($_GET['type'] ?? 'dynamic_url')));
        $campaigns = '<option value="0">Unassigned</option>';
        foreach ($this->campaigns->active() as $c) { $campaigns .= '<option value="' . esc_attr($c->id) . '">' . esc_html($c->name) . '</option>'; }
        $options = '';
        foreach ($this->types->all() as $key => $meta) { $options .= '<option value="' . esc_attr($key) . '" ' . selected($type, $key, false) . '>' . esc_html($meta['label']) . '</option>'; }
        return '<h1>New QR</h1><form class="qrb-card qrb-form" method="post">' . wp_nonce_field('qrbuzz_app_qr', 'nonce', true, false) . '<label>Name<input name="name" required></label><label>Type<select name="type">' . $options . '</select></label><label>URL / destination<input name="payload[destination_url]" type="url" placeholder="https://example.com"></label><fieldset><legend>WiFi</legend><label>SSID<input name="payload[ssid]"></label><label>Password<input name="payload[password]"></label><label>Encryption<select name="payload[encryption]"><option value="WPA">WPA/WPA2</option><option value="WEP">WEP</option><option value="NOPASS">None</option></select></label><label><input type="checkbox" name="payload[hidden]" value="1"> Hidden network</label></fieldset><fieldset><legend>Business Card</legend><label>First name<input name="payload[first_name]"></label><label>Last name<input name="payload[last_name]"></label><label>Organisation<input name="payload[organisation]"></label><label>Job title<input name="payload[job_title]"></label><label>Phone<input name="payload[phone]"></label><label>Email<input type="email" name="payload[email]"></label><label>Website<input type="url" name="payload[website]"></label><label>Address<textarea name="payload[address]"></textarea></label></fieldset><fieldset><legend>Email, Phone and SMS</legend><label>Email recipient<input type="email" name="payload[recipient]"></label><label>Email subject<input name="payload[subject]"></label><label>Email body<textarea name="payload[body]"></textarea></label><label>Phone / SMS number<input name="payload[phone]"></label><label>SMS message<textarea name="payload[message]"></textarea></label></fieldset><fieldset><legend>Location and Text</legend><label>Latitude<input name="payload[latitude]"></label><label>Longitude<input name="payload[longitude]"></label><label>Location label<input name="payload[label]"></label><label>Text<textarea name="payload[text]"></textarea></label></fieldset><label>Campaign<select name="campaign_id">' . $campaigns . '</select></label><button class="qrb-button qrb-button-primary">Create QR</button></form>'; }
    ...
    private function qrDetail(int $id): string {
        ...
        $png = wp_nonce_url(home_url('/app/qr/' . $qr->id . '/download/png'), 'qrbuzz_app_download_' . $qr->id);
        $svg = wp_nonce_url(home_url('/app/qr/' . $qr->id . '/download/svg'), 'qrbuzz_app_download_' . $qr->id);
        return '<div class="qrb-page-head"><h1>' . esc_html($qr->name) . '</h1><div class="qrb-actions"><a class="qrb-button" href="' . esc_url($png) . '">Download PNG</a><a class="qrb-button" href="' . esc_url($svg) . '">Download SVG</a></div></div><div class="qrb-card qrb-detail">' . $preview . '<div><p><strong>Type:</strong> ' . esc_html($this->types->label($qr->type)) . '</p><p><strong>Destination / payload:</strong> ' . esc_html($qr->destinationUrl ?: $qr->staticPayload) . '</p><p><strong>Scans:</strong> ' . esc_html((string) $qr->scanCount) . '</p><p><code>' . esc_html($qr->isTrackable() ? $this->generator->trackingUrl($qr->shortcode) : $qr->shortcode) . '</code></p></div></div>';
    }

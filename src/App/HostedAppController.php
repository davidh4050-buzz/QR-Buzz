...
class HostedAppController {
    ...
    private function app(string $title, string $content): void {
        ...
        echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . esc_html($title) . '</title>' . $this->styles() . $this->headAssets() . '</head><body class="qrb-body"><a class="qrb-skip-link" href="#qrb-main">Skip to content</a><div class="qrb-mobile-backdrop" data-qrb-drawer-close></div><div class="qrb-shell"><header class="qrb-topbar"><a class="qrb-brand qrb-brand-logo" href="/app/dashboard"><img src="' . esc_url(QR_BUZZ_URL . 'assets/qr-buzz-logo-header.png') . '" alt="QR Buzz"></a><button type="button" class="qrb-button qrb-mobile-menu" data-qrb-drawer-toggle aria-controls="qrb-sidebar" aria-expanded="false">' . $this->icon('menu') . '<span>Menu</span></button><div class="qrb-workspace"><span>Workspace</span><strong>' . esc_html($w->name) . '</strong></div><div class="qrb-topbar-actions"><a class="qrb-button qrb-button-primary" href="/app/qr/new">' . $this->icon('plus') . '<span>New QR</span></a><a class="qrb-button qrb-account-button qrb-icon-button" href="/app/settings/account" title="' . esc_attr($user->display_name ?: 'Account') . '" aria-label="Account settings">' . $this->icon('user') . '</a></div></header><aside class="qrb-sidebar" aria-label="Workspace navigation">' . $this->navigation() . '</aside><div class="qrb-app-frame"><main id="qrb-main" class="qrb-main">' . $this->flash() . $content . '</main></div></div>' . $this->appScripts() . $this->footerAssets() . '</body></html>'; exit;
    }

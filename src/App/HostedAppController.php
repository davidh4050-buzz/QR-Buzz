...
class HostedAppController {
    ...
    private function logoField(QRDesignSettings $design): string { $allowed = $this->entitlements->allows('logo_embedding'); $asset = $allowed && $design->logoAssetId ? $this->assets->find($design->logoAssetId) : null; $asset = $asset ? $this->assetStorage->withUrl($asset) : null; $legacy = $allowed && !$asset && $design->logoAttachmentId ? wp_get_attachment_image($design->logoAttachmentId, 'thumbnail') : ''; $image = $asset ? '<img src="' . esc_url($asset->url) . '" alt="">' : $legacy; $hasLogo = $asset || $legacy; return '<label>Logo<input type="hidden" class="qrb-logo-asset-id" name="logo_asset_id" value="' . esc_attr($allowed ? (string) $design->logoAssetId : '0') . '"><input type="hidden" class="qrb-logo-id" name="logo_attachment_id" value="' . esc_attr($allowed && !$asset ? (string) $design->logoAttachmentId : '0') . '"><span class="qrb-logo-actions"><button type="button" class="qrb-button qrb-select-logo" ' . disabled(!$allowed, true, false) . '>Choose from Asset Library</button><a class="qrb-button" href="' . esc_url(home_url('/app/assets')) . '">Manage assets</a><button type="button" class="qrb-button qrb-remove-logo" ' . disabled(!$allowed || !$hasLogo, true, false) . '>Remove logo</button></span><span class="qrb-logo-preview">' . wp_kses_post($image) . '</span><span class="qrb-help">PNG, JPG and WebP logo artwork is stored in your QR Buzz Asset Library. Non-square logos are fitted without stretching.</span></label>' . $this->assetPickerModal(); }

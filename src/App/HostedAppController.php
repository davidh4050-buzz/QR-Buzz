...
class HostedAppController {
    ...
    private function dashboard(): string {
        ...
        $html = '<section class="qrb-hero"><div class="qrb-page-header"><div><h1>' . esc_html($this->greeting()) . ', ' . esc_html(wp_get_current_user()->first_name ?: 'there') . '</h1><p>Create, connect and understand how people engage with your QR experiences.</p></div><div class="qrb-actions"><span class="qrb-badge">' . esc_html($entitlements['plan']['label']) . '</span><a class="qrb-button qrb-button-primary" href="/app/qr/new">New QR</a><a class="qrb-button" href="/app/campaigns">New Campaign</a></div></div></section>';
        ...
        $html .= '<section class="qrb-card"><h2>Quick actions</h2><div class="qrb-actions"><a class="qrb-button" href="/app/qr/new?type=dynamic_url&studio=1">Create Dynamic QR</a><a class="qrb-button" href="/app/qr/new?type=static_url&studio=1">Create Static QR</a><a class="qrb-button" href="/app/qr/new?type=wifi&studio=1">Create WiFi QR</a><a class="qrb-button" href="/app/qr/new?type=vcard&studio=1">Create Business Card QR</a><a class="qrb-button" href="/app/campaigns">Create Campaign</a></div></section>';
    ...
    private function analytics(): string {
        ...
        $html = $this->pageHeader('Analytics', 'Understand scan activity across your QR codes.', '<form><select name="range" onchange="this.form.submit()"><option value="today" ' . selected($rangeKey, 'today', false) . '>Today</option><option value="7days" ' . selected($rangeKey, '7days', false) . '>Last 7 days</option><option value="30days" ' . selected($rangeKey, '30days', false) . '>Last 30 days</option><option value="all" ' . selected($rangeKey, 'all', false) . '>All time</option></select></form>') . $this->metrics([['Total scans',$stats['total_scans']],['Scans today',$stats['scans_today']],['Last 7 days',$stats['scans_last_7_days']],['Latest scan',$stats['latest_scan'] ?: '-']]);
    ...
    private function workspaceSettings(): string { $w = $this->workspaces->current(); return $this->settingsNav('workspace') . $this->pageHeader('Workspace and Brand Kit', 'Set the workspace identity and defaults used across QR Buzz.') . '<form class="qrb-card qrb-form" method="post">' . wp_nonce_field('qrbuzz_app_workspace', 'nonce', true, false) . '<fieldset><legend>Workspace</legend><label>Name<input name="name" value="' . esc_attr($w->name) . '"></label><label>Website<input name="website_url" value="' . esc_attr($w->websiteUrl) . '"></label><label>Timezone<input name="timezone" value="' . esc_attr($w->timezone ?: wp_timezone_string()) . '"></label><label>Intended use<input name="intended_use" value="' . esc_attr($w->intendedUse) . '"></label></fieldset><fieldset><legend>Brand Kit foundation</legend><p>QR Studio uses your saved workspace and design defaults when new QR codes are created. Deeper brand controls will continue to expand from this area.</p></fieldset><button class="qrb-button qrb-button-primary">Save workspace</button></form>'; }
    private function billingSettings(): string { $e = $this->entitlements->summary(); $sub = $e['subscription']; return $this->settingsNav('billing') . $this->pageHeader('Billing', 'Review plan status and usage limits.') . '<div class="qrb-card"><p><strong>Current plan:</strong> ' . esc_html($e['plan']['label']) . '</p><p><strong>Status:</strong> ' . esc_html($sub['status']) . '</p><p><strong>Renewal:</strong> ' . esc_html($sub['renewal_date'] ?: '-') . '</p><p class="qrb-alert">Stripe is in test-mode prototype configuration for v0.9.8. If test keys are not configured, paid plan selection runs in local prototype mode.</p><a class="qrb-button" href="/app/onboarding?step=plan">Change plan</a></div>' . $this->usageCard(); }
    ...
    private function statusBadge(string $status): string { $class = $status === 'active' ? 'qrb-badge-success' : ($status === 'expired' ? 'qrb-badge-danger' : 'qrb-badge-warning'); return '<span class="qrb-badge ' . esc_attr($class) . '">' . esc_html(ucfirst($status)) . '</span>'; }
    ...
    private function qrCard(QRCode $qr): string { $payload = $this->payloads->payloadForQrCode($qr); try { $preview = '<img class="qrb-preview" src="' . esc_attr($this->generator->generatePngDataUri($payload, 180, QRDesignSettings::fromQrCode($qr))) . '" alt="QR preview">'; } catch (\Throwable $e) { $preview = '<p>Preview unavailable.</p>'; } $delete = '<form class="qrb-qr-card-delete" method="post" action="' . esc_url(home_url('/app/library')) . '">' . wp_nonce_field('qrbuzz_app_delete_qr_' . $qr->id, 'nonce', true, false) . '<input type="hidden" name="qr_id" value="' . esc_attr((string) $qr->id) . '"><button class="qrb-button qrb-icon-button" type="submit" aria-label="Delete ' . esc_attr($qr->name) . '" title="Delete QR code" data-confirm="Delete ' . esc_attr($qr->name) . '? This will permanently delete the QR code and its scan history.">' . $this->icon('trash') . '</button></form>'; return '<article class="qrb-card qrb-qr-card">' . $delete . '<div class="qrb-qr-card-preview">' . $preview . '</div><div><h2><a href="' . esc_url(home_url('/app/qr/' . $qr->id)) . '">' . esc_html($qr->name) . '</a></h2><div class="qrb-card-meta"><span>' . esc_html($this->types->label($qr->type)) . '</span><span>' . esc_html($qr->campaignName ?: 'Unassigned campaign') . '</span><span>' . esc_html((string) $qr->scanCount) . ' scans</span><span>Updated ' . esc_html($qr->updatedAt) . '</span></div></div><div class="qrb-actions">' . $this->statusBadge($qr->effectiveStatus()) . '<a class="qrb-button" href="' . esc_url($this->studioUrl($qr->type, $qr->id)) . '">Edit</a><a class="qrb-button" href="' . esc_url($this->downloadUrl($qr->id, 'png')) . '">PNG</a></div></article>'; }
    ...
    private function app(string $title, string $content): void {
        ...
        echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . esc_html($title) . '</title>' . $this->styles() . $this->headAssets() . '</head><body class="qrb-body"><a class="qrb-skip-link" href="#qrb-main">Skip to content</a><div class="qrb-mobile-backdrop" data-qrb-drawer-close></div><div class="qrb-shell"><header class="qrb-topbar"><a class="qrb-brand" href="/app/dashboard"><span class="qrb-brand-mark">QR</span><span>QR Buzz</span></a><button type="button" class="qrb-button qrb-mobile-menu" data-qrb-drawer-toggle aria-controls="qrb-sidebar" aria-expanded="false">' . $this->icon('menu') . '<span>Menu</span></button><div class="qrb-workspace"><span>Workspace</span><strong>' . esc_html($w->name) . '</strong></div><div class="qrb-topbar-actions"><a class="qrb-button" href="/app/qr/new">' . $this->icon('plus') . '<span>New QR</span></a><a class="qrb-button" href="/app/settings/billing">Billing</a><a class="qrb-button" href="/logout">' . esc_html($user->display_name ?: 'Logout') . '</a></div></header><aside class="qrb-sidebar" aria-label="Workspace navigation">' . $this->navigation() . '</aside><div class="qrb-app-frame"><main id="qrb-main" class="qrb-main">' . $this->flash() . $content . '</main></div></div>' . $this->appScripts() . $this->footerAssets() . '</body></html>'; exit;
    }
    ...
    private function flash(): string {
        $messages = ['deleted'=>'QR code deleted.','rule_deleted'=>'Smart Destination rule deleted.','rule_toggled'=>'Smart Destination rule status updated.','reordered'=>'Rule priority updated.','duplicated'=>'Rule duplicated as inactive.','created'=>'Created successfully.','welcome'=>'Welcome to your QR Buzz workspace.','saved'=>'Changes saved.'];
        $message = ''; foreach ($messages as $key => $copy) { if (isset($_GET[$key])) { $message = $copy; break; } }
        if ($message === '') { return ''; }
        return '<div class="qrb-toast" role="status"><span>' . esc_html($message) . '</span><button class="qrb-notice-dismiss" type="button" data-qrb-dismiss aria-label="Dismiss notice">&times;</button></div>';
    }

...
class HostedAppController {
    ...
    private function studioContentFields(?QRCode $qr, string $type, array $payload): string {
        $html = '<details class="qrb-studio-section" data-qrb-accordion="content" open><summary>Content ' . $this->tooltip('Choose what this QR represents and how it is organised.') . '</summary><div class="qrb-studio-section-body"><label>Name<input name="name" value="' . esc_attr($qr ? $qr->name : '') . '" required></label><label>Type<select name="type">';
        ...
        $html .= '<details class="qrb-studio-section" data-qrb-accordion="destination" open><summary>' . esc_html($this->types->label($type)) . ' ' . $this->tooltip($type === 'dynamic_url' ? 'Dynamic QR codes use a QR Buzz tracking URL so destinations can change later.' : 'Static QR codes encode the final information directly and do not collect scan analytics.') . '</summary><div class="qrb-studio-section-body">' . $this->typeFields($type, $payload, $qr) . '</div></details>';
        return $html;
    }
    ...
    private function studioDesignFields(QRDesignSettings $design): string {
        $html = '<details class="qrb-studio-section" data-qrb-accordion="design" open><summary>Design ' . $this->tooltip('Adjust colours, quiet zone and logo settings. Always test highly styled QR codes before printing.') . '</summary><div class="qrb-studio-section-body"><label>Theme<select name="theme" class="qrb-theme-select">';
        ...
        $html .= '</select></label>' . $this->input('Quiet zone / margin', 'margin', (string) $design->margin, 'number') . $this->logoField($design) . $this->input('Logo size (%)', 'logo_size', (string) $design->logoSize, 'number') . $logoNote . '</div></details>';
        if (!$this->entitlements->allows('advanced_branding')) {
            return $html . '<details class="qrb-studio-section" data-qrb-accordion="advanced-style"><summary>Advanced style ' . $this->tooltip('Advanced styling changes module and finder shapes for branded QR codes.') . '</summary><div class="qrb-studio-section-body"><p class="qrb-alert">Advanced QR styling is available on Pro and Business plans.</p></div></details>';
        }
        $html .= '<details class="qrb-studio-section" data-qrb-accordion="advanced-style"><summary>Advanced style ' . $this->tooltip('Use these controls carefully. Low contrast and heavy styling can reduce scan reliability.') . '</summary><div class="qrb-studio-section-body">';
        ...
        return $html . '<p class="qrb-help">Stylised QR codes should be tested on phones before printing, especially when using logos or low contrast colours.</p></div></details>';
    }
    ...
    private function qrHealthBlock(array $health): string { return '<div class="qrb-health qrb-health-' . esc_attr($health[0]) . ' qrb-qr-card-health"><strong>' . esc_html($health[1]) . ' ' . $this->tooltip('QR health is a simple deterministic check of status, fallback configuration and recent activity.') . '</strong><small>' . esc_html($health[2]) . '</small></div>'; }
    ...
    private function qrCard(QRCode $qr): string { $payload = $this->payloads->payloadForQrCode($qr); try { $preview = '<img class="qrb-preview" src="' . esc_attr($this->generator->generatePngDataUri($payload, 180, QRDesignSettings::fromQrCode($qr))) . '" alt="QR preview">'; } catch (\Throwable $e) { $preview = '<p>Preview unavailable.</p>'; } $status = $qr->effectiveStatus(); $health = $this->qrHealth($qr); $last = $qr->lastScan ? 'Last scanned ' . $qr->lastScan : 'No scans yet'; return '<article class="qrb-card qrb-qr-card" data-qrb-status="' . esc_attr($status) . '"><div class="qrb-qr-card-preview">' . $preview . '</div><div><h2><a href="' . esc_url(home_url('/app/qr/' . $qr->id)) . '">' . esc_html($qr->name) . '</a></h2><div class="qrb-card-meta"><span>' . esc_html($this->types->label($qr->type)) . ' · ' . esc_html($qr->campaignName ?: 'Unassigned campaign') . '</span><span>' . esc_html((string) $qr->scanCount) . ' scans · ' . esc_html($last) . '</span><span>Updated ' . esc_html($qr->updatedAt) . '</span></div>' . $this->qrHealthBlock($health) . '</div><div class="qrb-card-actions-primary">' . $this->statusBadge($status) . '<div class="qrb-actions"><a class="qrb-button" href="' . esc_url($this->studioUrl($qr->type, $qr->id)) . '">Edit</a><a class="qrb-button" href="' . esc_url($this->downloadUrl($qr->id, 'png')) . '">Download</a></div></div><div class="qrb-card-actions-secondary"><a class="qrb-button qrb-button-quiet" href="' . esc_url(home_url('/app/qr/' . $qr->id)) . '">Details</a>' . ($qr->isTrackable() ? '<a class="qrb-button qrb-button-quiet" href="' . esc_url(home_url('/app/qr/' . $qr->id)) . '#analytics">Analytics</a><a class="qrb-button qrb-button-quiet" href="' . esc_url(home_url('/app/qr/' . $qr->id)) . '#smart-destinations">Smart Destinations</a>' : '') . '</div></article>'; }
    ...
    private function flash(): string {
        if (!isset($_GET['saved']) && !isset($_GET['created']) && !isset($_GET['welcome'])) { return ''; }
        $message = isset($_GET['created']) ? 'Created successfully.' : (isset($_GET['welcome']) ? 'Welcome to your QR Buzz workspace.' : 'Changes saved.');

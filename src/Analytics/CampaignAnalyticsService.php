...
class CampaignAnalyticsService {
    ...
    public function stats(int $campaignId): array {
        return [
            'total_qr_codes' => $this->qrCount($campaignId),
            'dynamic_qr_codes' => $this->qrCount($campaignId, true),
            'static_qr_codes' => $this->qrCount($campaignId, false),
            'total_scans' => $this->scanCount($campaignId),
            'scans_today' => $this->scanCount($campaignId, DateRange::fromRequest('today')),
            'scans_last_7_days' => $this->scanCount($campaignId, DateRange::fromRequest('7days')),
            'scans_last_30_days' => $this->scanCount($campaignId, DateRange::fromRequest('30days')),
            'previous_7_days' => $this->previousPeriodCount($campaignId, 7),
            'top_qr_code' => $this->topQrCodes($campaignId, DateRange::fromRequest('all'), 1)[0] ?? null,
        ];
    }
    ...
    public function scanCount(int $campaignId, ?DateRange $range = null): int {
        global $wpdb;

        [$where, $params] = $this->scanWhere($campaignId, $range);
    ...
    public function scanCountsByDay(int $campaignId, DateRange $range): array {
        ...
        $rows = $wpdb->get_results($wpdb->prepare('SELECT DATE(s.scanned_at) AS scan_date, COUNT(*) AS scan_count FROM ' . Schema::scansTable() . ' s INNER JOIN ' . Schema::qrcodesTable() . ' q ON q.id = s.qr_id WHERE q.campaign_id = %d AND s.scanned_at BETWEEN %s AND %s GROUP BY DATE(s.scanned_at) ORDER BY scan_date ASC', $campaignId, $start, $end));
        $counts = [];
    ...
    public function topQrCodes(int $campaignId, DateRange $range, int $limit = 10): array {
        global $wpdb;
    ...
    public function recentScans(int $campaignId, DateRange $range, int $limit = 20): array {
        global $wpdb;
    ...
    public function referrerBreakdown(int $campaignId, DateRange $range): array {
        ...
        [$where, $params] = $this->scanWhere($campaignId, $range);
        return array_map(static fn($row): array => ['label' => $row->referrer ?: 'Direct / unknown', 'count' => (int) $row->scan_count], $wpdb->get_results($wpdb->prepare('SELECT s.referrer, COUNT(*) AS scan_count FROM ' . Schema::scansTable() . ' s INNER JOIN ' . Schema::qrcodesTable() . ' q ON q.id = s.qr_id ' . $where . ' GROUP BY s.referrer ORDER BY scan_count DESC LIMIT 10', ...$params)) ?: []);
    }
    ...
    public function previousPeriodCount(int $campaignId, int $days): int {
        global $wpdb;
        ...
        return (int) $wpdb->get_var($wpdb->prepare('SELECT COUNT(s.id) FROM ' . Schema::scansTable() . ' s INNER JOIN ' . Schema::qrcodesTable() . ' q ON q.id = s.qr_id WHERE q.campaign_id = %d AND s.scanned_at BETWEEN %s AND %s', $campaignId, $start, $end));
    }

    private function breakdown(int $campaignId, DateRange $range, string $parserMethod): array {
        global $wpdb;
    ...
    private function scanWhere(int $campaignId, ?DateRange $range): array {
        $where = ' WHERE q.campaign_id = %d';
        $params = [$campaignId];

...
class PlatformAdminController {
    ...
    private function page(string $title, string $content, int $status = 200): void { status_header($status); nocache_headers(); echo '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . esc_html($title) . ' · QR Buzz Platform</title>' . $this->styles() . '</head><body class="qrb-body qrb-platform"><a class="qrb-skip-link" href="#qrb-main">Skip to content</a><div class="qrb-shell"><aside class="qrb-sidebar"><a class="qrb-brand" href="/platform-admin/dashboard"><span class="qrb-brand-mark">QR</span><span>Platform Admin</span></a>' . $this->navigation() . '</aside><div class="qrb-app-frame"><header class="qrb-topbar"><div class="qrb-workspace"><span>Environment</span><strong>' . esc_html($this->repo->environmentLabel()) . '</strong></div><div class="qrb-topbar-actions"><span class="qrb-badge">v' . esc_html(QR_BUZZ_VERSION) . '</span><span class="qrb-badge">' . esc_html($this->stripeMode()) . '</span><a class="qrb-button" href="/app/dashboard">Workspace</a><a class="qrb-button" href="/logout">' . esc_html(wp_get_current_user()->display_name ?: 'Logout') . '</a></div></header><main id="qrb-main" class="qrb-main">' . $content . '</main></div></div></body></html>'; exit; }
    ...
    private function styles(): string { return '<style>' . $this->asset('assets/css/tokens.css') . $this->asset('assets/css/reset.css') . $this->asset('assets/css/base.css') . $this->asset('assets/css/typography.css') . $this->asset('assets/css/layout.css') . $this->asset('assets/css/components.css') . $this->asset('assets/css/forms.css') . $this->asset('assets/css/tables.css') . $this->asset('assets/css/utilities.css') . $this->asset('assets/css/pages/dashboard.css') . $this->asset('assets/css/pages/analytics.css') . $this->asset('assets/css/responsive.css') . '.qrb-platform .qrb-sidebar{background:#1f2937}.qrb-platform .qrb-brand-mark{background:#0ea5e9}.qrb-inline-form{display:inline-flex;align-items:center;gap:.5rem;flex-wrap:wrap}.qrb-inline-form select{min-height:40px}.qrb-confirm{display:inline-flex!important;grid-template-columns:none!important;align-items:center;gap:.35rem;font-size:12px}.qrb-card textarea[readonly]{width:100%;font-family:ui-monospace,Consolas,monospace}.qrb-table-wrap{overflow:auto}</style>'; }

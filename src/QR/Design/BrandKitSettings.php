...
class BrandKitSettings {
    ...
    public const OPTION = 'qrbuzz_brand_kit';

    private QRThemeRegistry $themes;

    public function __construct(?QRThemeRegistry $themes = null) {
    ...
    public function designDefaults(): QRDesignSettings {
        ...
        return new QRDesignSettings(array_merge($theme->toArray(), [
            'foreground_color' => $settings['foreground_color'],
            'background_color' => $settings['background_color'],
            'error_correction' => $settings['default_error_correction'],
            'logo_attachment_id' => absint($settings['default_logo_attachment_id']),
        ]));
    }

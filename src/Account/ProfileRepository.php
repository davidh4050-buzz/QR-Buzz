...
class ProfileRepository {
    ...
    public function ensure(int $userId, bool $termsAccepted = false): void {
        ...
        if ($exists > 0) { return; }
        $wpdb->insert(Schema::profilesTable(), ['user_id' => $userId, 'email_verified' => 0, 'onboarding_status' => 'new', 'onboarding_step' => 'first_qr', 'terms_accepted_at' => $termsAccepted ? $now : null, 'created_at' => $now, 'updated_at' => $now], ['%d','%d','%s','%s','%s','%s','%s']);
    }
    ...
    public function markEmailVerified(int $userId): void { global $wpdb; $this->ensure($userId); $wpdb->update(Schema::profilesTable(), ['email_verified' => 1, 'verification_token_hash' => null, 'updated_at' => current_time('mysql')], ['user_id' => $userId], ['%d','%s','%s'], ['%d']); }
}

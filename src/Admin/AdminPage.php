...
class AdminPage {
    ...
    public function renderDashboardPage(): void {
        ...
        echo '</div><div class="qrbuzz-actions"><a class="button" href="' . esc_url(admin_url('admin.php?page=qr-buzz-codes')) . '">Open Library</a><a class="button" href="' . esc_url(admin_url('admin.php?page=qr-buzz-analytics')) . '">View Analytics</a><a class="button" href="' . esc_url(admin_url('admin.php?page=qr-buzz-settings')) . '">Brand Kit</a>';
        if (current_user_can('qrbuzz_manage_platform')) { echo '<a class="button" href="' . esc_url(home_url('/platform-admin/dashboard')) . '">Platform Admin</a>'; }
        echo '</div>';
    ...
    private function renderDesignFields(QRDesignSettings $design): void {
        ...
        echo '<p><label>Data module style<br>' . $this->simpleSelect('dot_style', $design->dotStyle, ['square' => 'Square', 'dot' => 'Dots', 'rounded' => 'Rounded']) . '</label></p>';
        echo '<p><label>Finder pattern style<br>' . $this->simpleSelect('finder_style', $design->finderStyle, ['square' => 'Square', 'rounded' => 'Rounded', 'circle' => 'Circle']) . '</label></p>';
        echo '<p><label>Finder centre style<br>' . $this->simpleSelect('finder_dot_style', $design->finderDotStyle, ['square' => 'Square', 'rounded' => 'Rounded', 'dot' => 'Dot']) . '</label></p>';
        echo '<p><label>Finder colour<br><input type="color" name="finder_color" value="' . esc_attr($design->finderColor ?: $design->foregroundColor) . '" /></label></p>';
        echo '<p><label>Caption<br><input type="text" maxlength="120" name="caption" value="' . esc_attr($design->caption) . '" /></label></p>';
        echo '<p><label>Caption colour<br><input type="color" name="caption_font_color" value="' . esc_attr($design->captionFontColor) . '" /></label></p>';
    ...
    private function simpleSelect(string $name, string $selected, array $options): string {
        $html = '<select name="' . esc_attr($name) . '">';
        foreach ($options as $value => $label) { $html .= '<option value="' . esc_attr((string) $value) . '" ' . selected($selected, $value, false) . '>' . esc_html((string) $label) . '</option>'; }
        return $html . '</select>';
    }

...
class AnalyticsPage {
    ...
    public function __construct(?AnalyticsRepository $analytics = null, ?QRGenerator $generator = null, ?QRRepository $qrCodes = null, ?PerQRAnalyticsService $perQrAnalytics = null, ?QRInsightService $insights = null, ?QRTypeRegistry $types = null) {
    ...
    private function renderSingle(int $qrId): void {
        ...
        echo '<p><a href="' . esc_url(admin_url('admin.php?page=qr-buzz')) . '">&larr; Back to QR Codes</a></p>';

        if (!$qrCode) {
            echo '<div class="notice notice-error"><p>QR code not found.</p></div></div>';
            return;
        }
    ...
    private function renderInsights(array $insights): void {
        echo '<div class="qrbuzz-section qrbuzz-insights"><h2>Insights</h2><ul>';
        foreach ($insights as $insight) { echo '<li>' . esc_html($insight) . '</li>'; }
        echo '</ul></div>';
    }

    private function renderRangeFilter(DateRange $range): void {
        echo '<form class="qrbuzz-filter" method="get"><input type="hidden" name="page" value="qr-buzz-analytics" /><label for="qrbuzz-range">Date range </label><select id="qrbuzz-range" name="range">';
        foreach (DateRange::options() as $key => $label) { echo '<option value="' . esc_attr($key) . '" ' . selected($range->key, $key, false) . '>' . esc_html($label) . '</option>'; }
        echo '</select> ';
    ...
    private function renderTrend(array $trend, DateRange $range): void {
        ...
        foreach ($trend as $point) {
            $height = $max > 0 ? max(2, (int) round(((int) $point['scans'] / $max) * 160)) : 2;
            echo '<div class="qrbuzz-bar-wrap"><span class="qrbuzz-bar" style="height:' . esc_attr((string) $height) . 'px" title="' . esc_attr($point['date'] . ': ' . $point['scans'] . ' scans') . '"></span><span class="qrbuzz-bar-label">' . esc_html(mysql2date('M j', $point['date'])) . '<br>' . esc_html((string) $point['scans']) . '</span></div>';
        }
        echo '</div></div></div>';
    }
    ...
    private function renderRecentActivity(array $recentScans): void {
        ...
        foreach ($recentScans as $scan) {
            $name = isset($scan->qr_name) ? (string) $scan->qr_name : 'This QR code';
    ...
    private function renderBreakdowns(array $deviceBreakdown, array $browserBreakdown): void {
        echo '<div class="qrbuzz-section"><h2>Breakdowns</h2><div class="qrbuzz-breakdowns">';
    ...
    private function renderBreakdownTable(string $title, array $rows): void {
        echo '<div><h3>' . esc_html($title) . '</h3><table class="widefat striped"><thead><tr><th>Type</th><th>Scans</th></tr></thead><tbody>';
        if (!$rows) { echo '<tr><td colspan="2">No data yet.</td></tr>'; }
        foreach ($rows as $row) { echo '<tr><td>' . esc_html($row['label']) . '</td><td>' . esc_html((string) $row['count']) . '</td></tr>'; }
        echo '</tbody></table></div>';
    }
